import React from 'react'

const AboutBlurb = () => {
    return (
        <div className="bubble">
          <br/>
          <div className="home-headline empower">
            We empower the women of Ecuador by teaching them to code.
          </div>
          <p>
          Código Ecuador is a nonprofit organization working to promote technology skills and computer programming careers<br/> for high school students in Ecuador, with a focus on female empowerment.
          </p>
          <p>We believe that improving access to computer science curricula promotes global equity,<br/> and that collective action makes great changes in this world.
          </p>

          <p>Read more about our work below.
          </p>

          <br/>
        </div>
    )
}

export default AboutBlurb
